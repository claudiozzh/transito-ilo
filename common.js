...
// Ruta real confirmada directamente contra OneDrive el 04/08/2026:
// Documents/Subgerencia de Programas Sociales ILO/AUTOMATIZACION_PADRON_NOMINAL/...
// "Documents" no se escribe (es la raíz implícita de /me/drive/root), pero
// faltaba TODO el tramo "Subgerencia de Programas Sociales ILO/" — por eso
// ninguna ruta encontraba nada, aunque se veía "correcta" a simple vista.
const CARPETA_RAIZ = "Subgerencia de Programas Sociales ILO/AUTOMATIZACION_PADRON_NOMINAL";
const REPORTES_BASE = `${CARPETA_RAIZ}/01_REPORTES_DIARIOS`;
const CARPETA_TRANSITOS = `${CARPETA_RAIZ}/04_TRANSITOS`;
// El nombre real de la carpeta es "05_NINOS_BAJA_ALTA" (confirmado contra
// OneDrive) — el orden de palabras estaba invertido.
const CARPETA_BAJAS_ALTAS = `${CARPETA_RAIZ}/05_NINOS_BAJA_ALTA`;
...
/* ================== FECHAS ================== */
/* OJO: la versión anterior de esta función calculaba la hora de Lima
   ajustando con getTimezoneOffset() del navegador — pero eso SOLO funciona
   si el navegador está en UTC+0. Si la computadora ya está configurada en
   hora de Lima (lo normal para alguien en Perú), el cálculo se cancelaba
   mal y devolvía la fecha en UTC puro, que de noche en Perú ya es "mañana".
   Bug real encontrado el 04/08/2026 — el sistema seleccionaba el 5 de
   agosto por defecto, un día adelantado. Ahora se usa la zona horaria de
   Lima de forma explícita, sin depender de cómo esté configurada la
   computadora que abre la página. */
function fechaHoyPeru() {
  ...
  return new Date(Date.UTC(obj.year, obj.month - 1, obj.day, obj.hour, obj.minute, obj.second));
}
